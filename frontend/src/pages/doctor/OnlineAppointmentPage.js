...
const OnlineAppointmentPage = () => {
  ...
  useEffect(() => {
    const initializeWebRTC = async () => {
      try {
        if (!user || !patient) {
          console.error('User or patient not available');
          return;
        }
        ...
        setNotification({
          open: true,
          message: `Connecting to ${patient.name || 'patient'}...`,
          severity: 'info'
        });

        // Initialize WebRTC service
        const roomId = `call-${user._id || user.id}-${patient._id || patient.id}`;
        WebRTCService.initialize(user._id || user.id, patient._id || patient.id, roomId);
        ...
        WebRTCService.setCallbacks({
          onRemoteStream: (stream) => {
            ...
            setNotification({
              open: true,
              message: `Connected to ${patient.name || 'patient'}`,
              severity: 'success'
            });
            ...
            setMessages(prev => [
              ...prev,
              {
                id: Date.now(),
                sender: 'system',
                content: 'Call connected. You can now communicate with the patient.',
                timestamp: new Date()
              }
            ]);
          },
  ...
  }, [patient, user, audioOnly, navigate]);
    ...
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <Typography variant="h6">
          {audioOnly ? 'Voice Call' : 'Video Call'} with {patient?.name || 'Patient'}
        </Typography>
        ...
        >
          {connecting ? (
            <Box sx={{ textAlign: 'center', color: 'white' }}>
              ...
              <Typography>Connecting to {patient?.name || 'Patient'}...</Typography>
            </Box>
          ) : callStatus === 'ended' ? (
            <Box sx={{ textAlign: 'center', color: 'white' }}>
                ...
                >
                  <Paper
                    sx={{
                      p: 1.5,
                      borderRadius: 2,
                      bgcolor: msg.sender === 'doctor' ? theme.palette.primary.main :
                              msg.sender === 'system' ? alpha(theme.palette.info.main, 0.1) :
                              theme.palette.grey[100],
                      color: msg.sender === 'doctor' ? 'white' :
        ...
        >
          <CardContent>
            ...
            <Button
              ...
              fullWidth
              sx={{ mt: 2 }}
              onClick={() => navigate(`/doctor/patient/${patient?._id || patient?.id}/records`)}
            >
      ...
      >
        ...
        <Tooltip title="Open Chat">
          ...
          >
            <Badge color="error" badgeContent={chatOpen ? 0 : messages.filter(m => m.sender === 'patient').length}>
